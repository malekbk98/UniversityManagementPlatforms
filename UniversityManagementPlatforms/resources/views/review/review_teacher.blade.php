                      ...
                      <td>{{$teacher->user->last_name}}</td>
                        ...
                        <td width='20%' style="color:orange">
                          ...
                          <span class="badge badge-warning">No Reviews</span>
                            @else
                              @for($i=0;$i<round($teacher->total_review/$teacher->nbr_review);$i++)
                                <i class="fas fa-star"></i>
                              @endfor
                          @endif
                        </td>
                        <td>
                        <button class="btn btn-info" data-toggle="modal" data-target="#exampleModal" onclick=" model( {{ $teacher->id}} )">Add Review</button>
                      </td>
                      </tr>
                      @endforeach
                    </tbody>
      ...
      <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form action="{{ route('review_Teacher.add_Teacher_review') }}" method="post">
              @csrf
